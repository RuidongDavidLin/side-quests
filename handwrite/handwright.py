...
template = Template(
    background=Image.open('1.jpg'),  # 自定义背景图片
    # font_size=100,
    font=ImageFont.truetype("YunFengJingJingTi-2.ttf", size= 40), # 字体选择手写体
    line_spacing=45,
    ...
    bottom_margin=100,
    word_spacing=1,
    line_spacing_sigma=0.6,  # 行间距随机扰动
    font_size_sigma=0,  # 字体大小随机扰动
    word_spacing_sigma=0.3,  # 字间距随机扰动
    end_chars="，。",  # 防止特定字符因排版算法的自动换行而出现在行首
    perturb_x_sigma=0.4,  # 笔画横向偏移随机扰动
    perturb_y_sigma=0.4,  # 笔画纵向偏移随机扰动
